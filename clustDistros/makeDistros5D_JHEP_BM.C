...
// .L makeDistros5D_JHEP_BM.C++
// makeDistros5D_JHEP_BM()
/////////////
...
const int ns = 13; //297;   //number of samples
string option = "_13TeV"; //debug
const int nev = 100000;	  //number of ev per sample

string inputPath = "/eos/user/a/acarvalh/asciiHH_tofit/GF_HH_BSM/GF_HH_";  //folder with ascii lhe files (outside 'git' area)
...
//string fileslist_st = "samples_JHEP.txt"; //ascii names list !!!!!!!!!!!!!!!!
...
void makeDistros5D_JHEP_BM(){
  ...
  sstr << "Distros_" << np << "p_SM3M_12_JHEP_BM_100000_5D" << option;
  ...
  for(int f=0; f<ns; ++f)  { 
      ...
      Float_t binsxM22[56]  = { 250,260,270,280,290,300,310,320,330,340,
      ...
      Float_t binsySim2[5]  = {  0.0,0.4,0.6,0.8, 1.0};
      sprintf (htitle3,"H%sbin2",samplename.c_str()); //debug
      TH2D *bin2 = new TH2D(htitle3, htitle3,55,binsxM22,4,binsySim2);
  ...
  //for(int f=0; f<ns; ++f) { // ns
    ...
    ////////////////////////////////
    string filename;
    //if(f==0) {filename = "/eos/user/a/acarvalh/asciiHH_tofit/GF_HH_benchmarks/GF_3M.decayed"; }
    //else {
   ...
   // 1 = higgs1; 2 = higgs2;
    ...
    for (int k=0; k<Nev; ++k) {  // loop on number of events
	  ...
	  ev[f][k].pzh=-ev[f][k].pzh;
       ...
       //star angle computation           
        ...
        bin2->Fill(P12.M(),abs(costhetast)); 
    ...
    // if(f ==0 || f ==12) { 
      ...
      bin3->Write();
      bin4->Write();
	  cout << " Finishing processing SM" << endl;
      //bin2[nhist].Write();    
