...
int colors[] = {21,21,21,21,21,
    ...
    /*kOrange+2, kBlue, kBlack, kCyan+2, kViolet+2, kGreen+2, kPink,kMagenta,
                kSpring+8,   kYellow+2, kMagenta-6, kGreen-6, kOrange-6, kBlue-6, kCyan-6, kGreen-6, kPink-6, kRed-6,//18
                kMagenta-8, kOrange-8, kBlue-8, kCyan-8, kGreen-8, kPink-8, kRed-8,
                kMagenta-4, kViolet-8, kAzure-8, kSpring-8, kYellow-8, kGreen-8,
                kViolet-4, kAzure-4, kSpring-4, kYellow-4, kGreen-4}; //36
     */
const int maxCol = 36;
...
string mapNamefile = "list_all_translation.txt"; //debug
...
void draw(std::vector<TH1F*> h,
	  ...
	  TString option = "", int nclus = 99, bool allLegend = false, TString hName = "") {  //double ymin_ratio, double ymax_ratio,
  ...
  if(legHeader == "BENCHMARKS COMPARISON") {
    legxmin = (legRIGHT ? 0.85 : 0.28);
    legxmax = legxmin+0.01;
    legymax = (legTOP ? 0.90 : 0.55);
    legymin = legymax-3.55;     
  }
  else { 
    legxmin = (legRIGHT ? 0.50 : 0.18);
    legxmax = legxmin+0.80;
    legymin = (legTOP ? 0.82 : 0.50); //0.82
    legymax = legymin-0.32; //+0.08
  }
  ...
  ymax = ymax*1.2;
  vector <int> bin260content;  
  vector <int> bin350content;  
  vector <int> bin600content; 
  for (size_t i=0; i<h.size(); i++) {
    ...
    if( hName == "mhh"){  
      ...
      Int_t bin260 = xaxis->FindBin(270);
      Int_t bin350 = xaxis->FindBin(400);
      Int_t bin600 = xaxis->FindBin(600);
      //cout<<hName<<" hName"<<endl;
      double high260 = h[i]->GetBinContent(bin260);
      double high350 = h[i]->GetBinContent(bin350);
      double high600 = h[i]->GetBinContent(bin600);
      bin260content.push_back(high260);
      bin350content.push_back(high350);
      bin600content.push_back(high600);
      //cout << hName <<" bin260 "<<high260<<" bin350 "<<high350<<" bin600 "<<high600<<endl;
      }
    //////////////////////
    string nam = "";
    if(legHeader == "BENCHMARKS COMPARISON" || allLegend){
      ...
      h[i]->SetLineColor(1); //to print benchmark always black.
      h[i]->SetLineWidth(4); 
      nam = translate(clu[nclus][0].c_str());
  ...
  ////////
  if( hName == "mhh"){ 
     min260 = TMath::LocMin(bin260content.size(), &bin260content[0]);
     min350 = TMath::LocMin(bin350content.size(), &bin350content[0]);
     min600 = TMath::LocMin(bin600content.size(), &bin600content[0]);
     //
     max260 = TMath::LocMax(bin260content.size(), &bin260content[0]);
     max350 = TMath::LocMax(bin350content.size(), &bin350content[0]);
     max600 = TMath::LocMax(bin600content.size(), &bin600content[0]);
     cout << " Minimum " <<" bin260 "<<min260 << " " << bin260content[min260]<<" bin350 "<<min350<< " " << bin260content[min350]<<" bin600 "<<min600<< " " << bin600content[min260]<<endl;
     cout << " Maximum " <<" bin260 "<<max260 << " " << bin260content[max260]<<" bin350 "<<max350<< " " << bin260content[max350]<<" bin600 "<<max600<< " " << bin600content[max260]<<endl;
     }
    string nam = "";
     for (size_t i=0; i<h.size(); i++) {
         nam = translate(clu[nclus][i].c_str());
         if(i==min260) {
             h[i]->SetLineColor(2); 
             h[i]->SetLineWidth(3); 
             h[i]->Draw(options);
             //h[i]->SetLineStyle(2);
             leg->AddEntry(h[i],nam.c_str(),"l");
         }
         if(i==max260) {
             h[i]->SetLineColor(46); 
             h[i]->SetLineWidth(3);
             h[i]->Draw(options);
             //h[i]->SetLineStyle(2);
             //cout<<" teste "<<translate(clu[nclus][i].c_str())<<" "<<nclus<<" "<<i <<endl;
             leg->AddEntry(h[i],nam.c_str(),"l");
         }
         if(i==min350) {
             h[i]->SetLineColor(3); 
  ...
  drawPrivate(0.025); //Ncl..
    ...
    if( hName == "mhh"){ 
        cout<<" draw line "<<endl;
        TLine *line270 = new TLine(270,can->GetUymin(),270,can->GetUymax());
        line270->Draw("same");
        TLine *line400 = new TLine(400,can->GetUymin(),400,can->GetUymax()-500);
        line400->Draw("same");
        TLine *line600 = new TLine(600,can->GetUymin(),600,can->GetUymax()-300);
        line600->Draw("same");
    } 
...
cout << size << endl; //debug
  ...
  //std::cout << " Getting " << fname << std::endl;
    ...
    if(nsam<maxCol){ histo->SetLineColor(colors[nsam]);
        ...
        histo->SetLineColorAlpha(21, 0.25); //to get transparency!
  ...
  if(h.size()>0){
      if(!ratio){ draw(h, Outname+testoption+"_Clus"+sst.str()+"_"+hName,
 	   ...
 	   scale,rebin,orbin,option,nc, allLeg,hName);  //,ymin_ratio,ymax_ratio
      }
      else {
...
// mhh needs to be first
void plot_mhh(bool rat, int ncluster = 99, int rebin = 2, TString opt="") {
    performancePlot1D(rat,ncluster-1,"mhh",240.,1200.,0.,800.,"m_{hh} [GeV]","", false, false, false,-9.,rebin,200,opt);
}
