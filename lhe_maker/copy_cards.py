...
for linep in linesp:
    ...
    for token in tokens:
        ...
        #print(ll[counterp])
        if counterp == 0 : Vkl = float(num)
        if counterp == 1 : Vkt=float(num)
        if counterp == 2 : Vc2=float(num)
        if counterp == 3 : Vcg=float(num)
        if counterp == 4 : Vc2g=float(num)
        counterp += 1
        ########################################
    ...
    frun.write('cp runcmsgrid_PDF4LHC.sh '+dirrun+'\n'+\
               'cd '+dirrun+"\n"+\
               './runcmsgrid.sh 50000 '+str(randint(0,10000)) +' 10 > output_'+base+str(countline)+'.txt &\n'+\
               'cd - \n')
    countline+=1    
fprocess.close()    
fsetup.close()
